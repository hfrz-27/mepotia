...
export const DEMO_PRODUCTS = [
  {
    ...
    is_featured: true,
    status: "published",
    product_images: [{ id: "d1", url: IMG("photo-1511707171634-5f897ff02aa9"), sort_order: 0 }],
    demo: true,
  },
  {
    id: "demo-product-2",
  ...
  },
  {
    ...
    is_featured: false,
    status: "published",
    product_images: [{ id: "d4", url: IMG("photo-1505740420928-5e560c06d30e"), sort_order: 0 }],
    demo: true,
  },
  {
    id: "demo-product-5",
...
export function fillProducts(real = [], limit = 6) {
  ...
  for (const demo of DEMO_PRODUCTS) {
    ...
    if (!usedTitles.has(demo.title)) {
      items.push(demo);
      usedTitles.add(demo.title);
    }
  }

  // 20’lik grid vb. için demo seti yetmezse kopyalarla doldur
  let n = 0;
  while (items.length < limit && DEMO_PRODUCTS.length) {
    const base = DEMO_PRODUCTS[n % DEMO_PRODUCTS.length];
    items.push({
      ...base,
      id: `${base.id}-fill-${items.length}`,
      title: base.title,
      demo: true,
    });
    n += 1;
