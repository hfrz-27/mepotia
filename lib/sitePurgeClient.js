import { deleteAllProductStorage } from "@/lib/deleteProduct";

/**
 * Admin oturumu ile tarayıcıdan tüm vitrin içeriğini siler.
 * Service role gerekmez — RLS admin politikası yeterli.
 */
export async function deleteAllRows(supabase, table, idColumn = "id") {
  let total = 0;
...
export async function purgeSiteFromBrowser(supabase) {
  const deletedProducts = await deleteAllRows(supabase, "products");
  const deletedStorageFiles = await deleteAllProductStorage(supabase);
  const deletedNews = await deleteAllRows(supabase, "tech_posts");
  ...
  const deletedFeedback = await deleteAllRows(supabase, "site_feedback");
