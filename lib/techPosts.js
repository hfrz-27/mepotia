...
const POST_SELECT = "id, title, excerpt, body, cover_url, source_url, featured_order, is_featured, published, created_at, updated_at";
...
export async function getTechPosts({ limit = 6, page = null } = {}) {
  ...
  if (error?.message?.includes("featured_order")) {
    ...
    if (!legacy.error) return { data: (legacy.data || []).map((post) => ({ ...post, featured_order: post.is_featured ? 1 : null })), count: legacy.count ?? legacy.data?.length ?? 0, error: null };
  }
...
export async function getTechPostById(id) {
  ...
  if (error?.message?.includes("featured_order")) {
    const legacy = await supabase
      ...
      .gte("created_at", techPostsCutoffIso())
      .single();
    data = legacy.data ? { ...legacy.data, featured_order: legacy.data.is_featured ? 1 : null } : null;
    error = legacy.error;
  }
