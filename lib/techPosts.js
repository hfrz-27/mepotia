...
const POST_SELECT = "id, title, excerpt, body, cover_url, source_url, featured_order, trending_order, is_featured, published, created_at, updated_at";
...
export async function getTechPosts({ limit = 6, page = null, featuredFilter = "all", trendingOnly = false } = {}) {
  ...
  if (featuredFilter === "only") query = query.not("featured_order", "is", null);
  if (featuredFilter === "none") query = query.is("featured_order", null);
  if (trendingOnly) query = query.not("trending_order", "is", null);
  ...
  if (error?.message?.includes("featured_order") || error?.message?.includes("trending_order")) {
    ...
    if (!legacy.error) return { data: (legacy.data || []).map((post) => ({ ...post, featured_order: post.is_featured ? 1 : null, trending_order: null })), count: legacy.count ?? legacy.data?.length ?? 0, error: null };
  }
...
export async function getTechPostById(id) {
  ...
  if (error?.message?.includes("featured_order") || error?.message?.includes("trending_order")) {
    const legacy = await supabase
      ...
      .gte("created_at", techPostsCutoffIso())
      .single();
    data = legacy.data ? { ...legacy.data, featured_order: legacy.data.is_featured ? 1 : null, trending_order: null } : null;
    error = legacy.error;
  }
