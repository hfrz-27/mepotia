...
async function searchMepotiaListings(query) {
  ...
  if (error) {
    ...
    return { error: "Vitrin araması başarısız." };
  }
...
export async function lookupTechModelPrice(rawQuery, options = {}) {
  ...
  if (mepotiaResult.error) {
    return { error: mepotiaResult.error };
  }
  ...
  const market = marketResult?.error
    ? null
    : {
  ...
  const comparison = buildComparison(mepotia.referencePrice, market);
  ...
  return {
    ...
    hasData: Boolean(mepotia.summary || market || comparison),
  };
}
