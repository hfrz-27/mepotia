const DEFAULT_PHONE = "905059574122";
...
export async function recoverPhotosFromStorage(supabase) {
  ...
  for (const folder of folders || []) {
    ...
    if (!product) {
      const { error: insertErr } = await supabase.from("products").insert({
        id: productId,
        title: "Geri yüklenen ilan — düzenle",
        description:
          "Bu ilan otomatik geri yüklendi. Yönetim panelinden başlık, fiyat ve açıklamayı güncelle.",
        price: 1000,
        status: "published",
        condition: "used",
        source: "admin",
      ...
      if (insertErr) {
        console.error("product restore", productId, insertErr);
        continue;
      }
      productsRestored += 1;
    }
  ...
  return { linked, productsRestored, uid };
}
