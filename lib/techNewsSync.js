...
import { assertSafeExternalUrl } from "@/lib/safeExternalUrl";
import {
  cleanText,
  cleanTitle,
  decodeHtml,
...
async function tryDiscoverRss(pageUrl) {
  ...
  try {
    await assertSafeExternalUrl(pageUrl);
    ...
    if (res.ok) {
      ...
      if (rssHref) {
        ...
        await assertSafeExternalUrl(absolute);
...
const MAX_BODY_PARAGRAPHS = 15;
const MIN_PARAGRAPH_CHARS = 35;

function extractArticleHtml(html) {
  const patterns = [
    /<article[\s\S]*?<\/article>/i,
    /<div[^>]+class="[^"]*\b(news-detail|post-content|article-content|entry-content|detail-content|haber-icerik|content-body|newsContent)[^"]*"[\s\S]*?<\/div>/i,
    /<div[^>]+id="(?:article|content|news-content|haber|news-detail)"[^>]*[\s\S]*?<\/div>/i,
  ];

  for (const pattern of patterns) {
    const match = html.match(pattern);
    if (match?.[0] && match[0].length > 400) return match[0];
  }
...
function htmlToParagraphs(html) {
  ...
  return cleaned
    ...
    .filter((p) => p.length > MIN_PARAGRAPH_CHARS && !looksLikeCode(p));
}
...
async function fetchArticleParagraphs(url) {
  try {
    ...
    const html = await res.text();
    const articleMatch = extractArticleHtml(html);

    return sanitizeParagraphs(htmlToParagraphs(articleMatch)).slice(0, MAX_BODY_PARAGRAPHS);
  } catch {
    return [];
  }
}
...
async function resolveArticleBody(item) {
  ...
  let paragraphs = dedupeParagraphs([
    ...fromRss,
    ...fromPage.filter((p) => !isNearDuplicate(p, description)),
  ]).filter((p) => !isNearDuplicate(p, description));

  if (!paragraphs.length && description.length > MIN_PARAGRAPH_CHARS && !isJunkParagraph(description)) {
    paragraphs = [description];
  }

  return paragraphs.slice(0, MAX_BODY_PARAGRAPHS);
}
...
function buildBody(paragraphs, description, title) {
  ...
  let parts = paragraphs.length ? dedupeParagraphs(paragraphs) : [];

  if (desc.length > MIN_PARAGRAPH_CHARS && !isJunkParagraph(desc) && !parts.some((p) => isNearDuplicate(p, desc))) {
    parts = [desc, ...parts];
  }

  if (!parts.length) {
    parts = [cleanText(description) || cleanTitle(title)].filter((p) => !isJunkParagraph(p));
  }
