          ...
          {FAQS.map(([question, answer], index) => (
            <details key={question} className="group border-b border-black/10 last:border-b-0">
              ...
              <div className="pb-6 pl-0 pr-12 sm:pl-9">
                ...
                {index === FAQS.length - 1 ? <Link href="/sss" className="mt-4 inline-flex items-center text-[12px] font-semibold text-[#0071e3]">Tüm yardım konuları <ChevronRight className="h-4 w-4" /></Link> : null}
              </div>
            </details>
          ))}
        </div>
