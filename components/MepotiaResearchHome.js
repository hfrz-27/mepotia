...
function SellDeviceSection({ content }) {
  return (
    <section className="home-shell py-6 sm:py-12">
      <div className="relative overflow-hidden rounded-[24px] bg-[#ebe9e4] px-4 py-8 text-[#1d1d1f] ring-1 ring-black/[.045] sm:px-10 sm:py-14 lg:rounded-[36px] lg:px-16">
        ...
        <div className="relative mx-auto max-w-4xl text-center"><p className="text-[10px] font-semibold uppercase tracking-[.2em] text-[#0071e3] sm:text-[11px]">Mepotia’ya Sat</p><h2 className="mt-3 text-[2.25rem] font-semibold leading-[.94] tracking-[-.065em] sm:mt-5 sm:text-[5rem]">{content.sell_title}<br /><span className="text-black/35">{content.sell_accent}</span></h2><p className="mx-auto mt-4 max-w-xl text-[12px] leading-relaxed text-[#6e6e73] sm:mt-6 sm:text-[16px]">{content.sell_copy}</p></div>
        <div className="relative mx-auto mt-6 grid max-w-5xl gap-2.5 sm:mt-12 sm:gap-3 md:grid-cols-3">
          {[[ScanLine, "01", "Cihazını tanımla", "Modeli ve temel özellikleri seç."], [Camera, "02", "Durumunu göster", "Kondisyonu ve fotoğrafları ekle."], [WalletCards, "03", "Teklifini değerlendir", "Sana iletilen değeri net biçimde gör."]].map(([Icon, number, title, copy]) => <article key={number} className="group rounded-[18px] border border-white/80 bg-white/65 p-4 text-left backdrop-blur-xl transition duration-500 hover:-translate-y-1 hover:bg-white/85 sm:rounded-[24px] sm:p-7"><div className="flex items-center justify-between"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#1d1d1f] text-white sm:h-11 sm:w-11"><Icon className="h-4 w-4 sm:h-5 sm:w-5" strokeWidth={1.5} /></span><span className="text-[10px] font-semibold tracking-[.15em] text-[#86868b] sm:text-[11px]">{number}</span></div><h3 className="mt-3 text-[1.05rem] font-semibold tracking-[-.04em] sm:mt-8 sm:text-[1.35rem]">{title}</h3><p className="mt-1 text-[11px] leading-relaxed text-[#6e6e73] sm:mt-2 sm:text-[13px]">{copy}</p></article>)}
        </div>
        <div className="relative mx-auto mt-3 flex max-w-5xl items-center justify-between gap-3 rounded-[18px] bg-[#1d1d1f] p-4 text-white sm:mt-5 sm:rounded-[24px] sm:px-8 sm:py-6"><div className="min-w-0 text-left"><p className="text-[8px] font-semibold uppercase tracking-[.16em] text-white/45 sm:text-[10px]">Hazırsan başlayalım</p><p className="mt-1 text-[13px] font-semibold leading-tight tracking-[-.035em] sm:text-[1.25rem]">Cihazının teklif sürecini şimdi başlat.</p></div><Link href="/bana-sat" className="inline-flex h-10 shrink-0 items-center gap-1 rounded-full bg-[#0071e3] px-4 text-[12px] font-semibold transition hover:bg-[#0077ed] sm:h-12 sm:px-6 sm:text-[14px]">Teklif al <ChevronRight className="h-4 w-4" /></Link></div>
      </div>
    </section>
  );
}
...
export default function MepotiaResearchHome({ products = [], campaignImages = [], news = [], content: rawContent = {}, media = {} }) {
  const content = getHomeContent(rawContent);
      ...
      <section className="home-shell py-8 sm:py-12">
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-3">
          {FEATURES.map(([eyebrow, title, copy, image, href], i) => {
            const names = ["phone", "computer", "tablet"];
            const name = names[i];
            ...
            <Link
              ...
              className="group relative block h-[420px] min-w-0 overflow-hidden rounded-[26px] bg-white ring-1 ring-black/[.05] transition duration-500 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#0071e3] sm:h-[470px] lg:h-[520px] lg:rounded-[30px]"
            >
              <div
                className="absolute inset-0 bg-contain bg-center bg-no-repeat transition duration-[900ms] ease-out group-hover:scale-[1.018]"
                style={{ backgroundImage: `url("${image}")` }}
                role="img"
                aria-label={`${title} kategori görseli`}
              />
              <span className="sr-only">{eyebrow}: {title}. {copy}</span>
            </Link>
          )})}
        </div>
      </section>
          ...
          {SERVICES.map((baseItem, index) => { const item = { ...baseItem, ...(index === 0 ? { title: content.trade_title, copy: content.trade_copy, action: content.trade_action, image: media.trade || baseItem.image } : index === 1 ? { title: content.compare_title, copy: content.compare_copy, action: content.compare_action, image: media.compare || baseItem.image } : { title: content.request_title, copy: content.request_copy, action: content.request_action }) }; return index === 2 ? (
            <article key={item.title} className="relative overflow-hidden rounded-[28px] bg-[linear-gradient(135deg,#eef6ff_0%,#f7f4ff_48%,#edf9ff_100%)] px-5 py-11 text-center ring-1 ring-[#0071e3]/10 sm:px-10 sm:py-14 lg:min-h-[580px] lg:rounded-[36px]">
              ...
              <div className="relative mx-auto max-w-4xl">
                ...
                <h3 className="mx-auto mt-7 max-w-4xl text-[3rem] font-semibold leading-[.94] tracking-[-0.065em] text-[#1d1d1f] sm:text-[5rem]">{content.request_title}<br /><span className="bg-gradient-to-r from-[#0071e3] to-[#7d5cff] bg-clip-text text-transparent">{content.request_accent}</span></h3>
